...
app.post("/upload", upload.single("product"), (req, res) => {
  res.json({
    success: 1,
    image_url: `https://e-commerce-backend-1.onrender.com/images/${req.file.filename}`,
  });
});
...
app.post("/signup", async (req, res) => {
  ...
  if (check) {
    return res
      .status(400)
      .json({ success: false, errors: "Existing userFound email or password" });
  }
  let cart = {};
  for (let i = 0; i < 300; i++) {
    cart[i] = 0;
...
// creating popular women section
...
//..creating fetch user MIDDLEWARE
...
// creaing endpoint to get cartdata
